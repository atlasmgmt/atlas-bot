...
class VerificationEmbedCreation(discord.ui.View):
    ...
    async def EmbedFinished(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        module = await create_module_configuration(interaction.guild.id, "VerifyModule")  
...
class VerificationPanelChannel(discord.ui.ChannelSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        module = await create_module_configuration(self.mongo, "VerifyModule")
        db = self.mongo["Curly"]["Config"]
        await db.update_one({"guild_id": interaction.guild.id},{"$setOnInsert": {"VerifyModule": {}}}, upsert=True)
        await db.update_one({"guild_id": interaction.guild.id}, {"$set": {"VerifyModule.panel_channel_id": channel_id}})
        return
class VerificationLogChannel(discord.ui.ChannelSelect):
    def __init__(self, mongo_connection, ctx):
        self.ctx = ctx
        self.mongo = mongo_connection
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await db.update_one(
            ...
            {
                "$setOnInsert": {"VerifyModule": {}},  
                "$set": {"VerifyModule.log_channel_id": channel_id},
            },
            upsert=True
        )        
        return
...
class VerifiedRole(discord.ui.RoleSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await db.update_one(
            ...
            {
                "$setOnInsert": {"VerifyModule": {}},  
                "$set": {"VerifyModule.verified_role_id": role_id},
            },
            upsert=True
        )        
        return
...
class UnverifiedRole(discord.ui.RoleSelect):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await db.update_one(
            ...
            {
                "$setOnInsert": {"VerifyModule": {}},  
                "$set": {"VerifyModule.unverified_role_id": role_id},
            },
            upsert=True
        )        
        return
...
class ChannelsSelection(discord.ui.Select):
    def __init__(self, message, ctx, mongo):
        self.message = message
        self.mongo = mongo
        self.ctx = ctx
        options=[
            discord.SelectOption(label=f"Panel Channel", description=f"The channel that the verification panel sends to.", value="panel"),
            discord.SelectOption(label=f"Logs Channel", description=f"The channel that the verification logs send to.", value="log")
            ]
        super().__init__(placeholder="Channels",max_values=1,min_values=1,options=options)
    async def callback(self, interaction: discord.Interaction):
        if self.ctx.author.id != interaction.user.id:
            embed = discord.Embed(description="This is not your panel!", color=discord.Color.dark_embed())
            embed.set_author(icon_url=interaction.user.display_avatar.url)
        ...
        if self.values[0] == "panel":
            view = discord.ui.View(timeout=300)
            view.add_item(VerificationPanelChannel(ctx=self.ctx, mongo_connection=self.mongo))
            return await interaction.followup.send(view=view, ephemeral=True)
        
        if self.values[0] == "log":
            view = discord.ui.View(timeout=300)
            view.add_item(VerificationLogChannel(ctx=self.ctx, mongo_connection=self.mongo))
            return await interaction.followup.send(view=view, ephemeral=True)
...
class RoleSelection(discord.ui.Select):
    def __init__(self, message, ctx, mongo):
        ...
        self.ctx = ctx
        options=[
            discord.SelectOption(label=f"Verified Role", description=f"The role that verified users are given", value="verified"),
            discord.SelectOption(label=f"Unverified Role", description=f"The role that verified users are removed from", value="unverified")
            ]
        super().__init__(placeholder="Roles",max_values=1,min_values=1,options=options)
    async def callback(self, interaction: discord.Interaction):
        if self.ctx.author.id != interaction.user.id:
            embed = discord.Embed(description="This is not your panel!", color=discord.Color.dark_embed())
            embed.set_author(icon_url=interaction.user.display_avatar.url)
            return await interaction.response.send_message(embed=embed, ephemeral=True)
        await interaction.response.defer(ephemeral=True)
        if self.values[0] == "verified":
            view = discord.ui.View(timeout=300)
            view.add_item(VerifiedRole(ctx=self.ctx, mongo_connection=self.mongo))
            return await interaction.followup.send(view=view, ephemeral=True)
        
        if self.values[0] == "unverified":
            view = discord.ui.View(timeout=300)
            view.add_item(UnverifiedRole(ctx=self.ctx, mongo_connection=self.mongo))
            return await interaction.followup.send(view=view, ephemeral=True)
...
class EmbedCreation(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await message.edit(view=VerificationEmbedCreation(timeout=None, ctx=self.ctx, message=message, mongo=self.mongo))
...
class VerificationView(discord.ui.View):
    def __init__(self, ctx,mongo_connection, message):
        ...
        self.ctx = ctx
        self.mongo = mongo_connection
        #self.verification_panel_channel = VerificationPanelChannel(ctx=self.ctx, mongo_connection=mongo_connection)
        #self.verification_log_channel = VerificationLogChannel(ctx=self.ctx, mongo_connection=mongo_connection)
        #self.verified_role = VerifiedRole(ctx=self.ctx, mongo_connection=mongo_connection)
        #self.unverified_role = UnverifiedRole(ctx=self.ctx, mongo_connection=mongo_connection)
        self.channel_view = ChannelsSelection(message=self.message, ctx=self.ctx, mongo=self.mongo)
        self.roles_view = RoleSelection(message=self.message, ctx=self.ctx, mongo=self.mongo)
        ...
        #self.add_item(item=self.verification_panel_channel)
        #self.add_item(item=self.verification_log_channel)
        #self.add_item(item=self.verified_role)
        #self.add_item(item=self.unverified_role)
        self.add_item(item=self.channel_view)
        self.add_item(item=self.roles_view)
